...
def create_table(db_name="speech_kit.db", table_name='messages'):
    try:
...
def insert_row(user_id, message, tts_symbols, db_name="speech_kit.db", table_name='messages'):
    try:
        # Подключаемся к базе
        with sqlite3.connect(db_name) as conn:
            cursor = conn.cursor()
            # Вставляем в таблицу новое сообщение
            cursor.execute(f'''INSERT INTO {table_name} (user_id, message, tts_symbols)VALUES (?, ?, ?)''',
